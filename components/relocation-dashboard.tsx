...
export function RelocationDashboard() {
  ...
  const [profile] = useState<RelocationProfile>(() => typeof window === "undefined" ? defaultProfile : { ...defaultProfile, ...readStored(storageKeys.profile, defaultProfile) });
  const [tasks, setTasks] = useState<WorkspaceTask[]>(() => typeof window === "undefined" ? buildTasks(defaultProfile) : readStored(storageKeys.tasks, buildTasks(readStored(storageKeys.profile, defaultProfile))));
  const [documents, setDocuments] = useState<WorkspaceDocument[]>(() => typeof window === "undefined" ? baseDocuments : readStored(storageKeys.documents, baseDocuments));
  const [budgetItems, setBudgetItems] = useState<WorkspaceBudget[]>(() => typeof window === "undefined" ? baseBudget : readStored(storageKeys.budget, baseBudget));
  ...
  if (!mounted) return <main className="dashboard-shell workspace-loading" aria-busy="true" />;
